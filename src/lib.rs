...
fn generate_websocket_handler(
    ...
    quote! {
        ...
        hyperware_process_lib::http::server::HttpServerRequest::WebSocketOpen { path, channel_id, source_socket_addr } => {
            ...
            match hyperware_process_lib::hyperapp::get_server() {
                Some(server) => server.handle_websocket_open(&path, channel_id, source_socket_addr),
